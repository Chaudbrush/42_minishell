...
static int	pipe_syntax_err(t_pipe *pipe_node)
{
	t_exec	*right_exec;
	t_exec	*left_exec;

	right_exec = (t_exec *)(pipe_node->right);
	left_exec = (t_exec *)(pipe_node->left);
	if ((pipe_node->left->type == EXEC && !left_exec->argv[0])
		|| (pipe_node->right->type == EXEC && !right_exec->argv[0]))
...
static t_cmd	*parsepipe(char **str, char *end_str, t_list **cmd_list)
{
	...
	if (ft_exists_wskip(str, end_str, "|"))
	{
		...
		if (!left_cmd)
			return (NULL);
		cmd = init_t_pipe(cmd, left_cmd, cmd_list);
