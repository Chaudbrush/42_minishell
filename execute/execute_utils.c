...
static void	read_line_heredoc(t_redir *redir_node, char *ptr)
{
	...
	shell()->doc_exp = 0;
	if (!ft_strchr(redir_node->file, '\"') && !ft_strchr(redir_node->file, '\''))
		shell()->doc_exp = 1;
	remove_quotes(redir_node->file, &shell()->expan_delim);
	while (1)
	{
		ptr = readline(">");
		if (heredoc_exit(ptr))
	...
	shell()->doc_exp = 0;
	close(hd_pipe[1]);
	redir_node->heredoc_fdin = hd_pipe[0];
